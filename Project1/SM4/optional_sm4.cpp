...
__m128i L_NI(uint32_t B) {
    ...
    return _mm_xor_si128(B1, L_B);
}

uint32_t T_NI(uint32_t x) {
    return _mm_extract_epi32(L_NI(S(x)), 0);
}
...
// GCM mode operations
void sm4_ctr(uint8_t key[16], uint8_t iv[16], uint8_t* plain, uint8_t* cipher, size_t length) {
    uint32_t rk[32];
    RoundKey(key, rk);
    uint8_t counter[16];
    ...
    for (size_t i = 0; i < num_blocks; i++) {
        EncryptBlock_table(counter, stream_block, rk);
        for (int j = 0; j < 16; j++)
            cipher[i * 16 + j] = plain[i * 16 + j] ^ stream_block[j];
...
void t_table_check() {
    ...
    cout << "Average time: " << total_time / 16 << " us" << endl;
}
...
void AES_NI_check() {
    ...
    cout << "Average encryption time: " << total_time / 16 << " us" << endl;
}
...
void GCM_check() {
    ...
    uint8_t key[16], iv[16], plaintext[16], aad[8];
